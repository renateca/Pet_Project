...
Suite* add_suite_cos(void) {
  Suite* suit = suite_create("cos");
  TCase* tc_core = tcase_create("Core_test_cos");
  ...
  tcase_add_test(tc_core, test_cos_negative);
  tcase_add_test(tc_core, test_cos_one_negative);
  tcase_add_test(tc_core, test_cos_huge_positive);
  tcase_add_test(tc_core, test_cos_huge_negative);
  tcase_add_test(tc_core, test_cos_big_mantissa);
  tcase_add_test(tc_core, test_cos_eps_positive);
  tcase_add_test(tc_core, test_cos_eps_negative);
  tcase_add_test(tc_core, test_cos_inf);
  tcase_add_test(tc_core, test_cos_nan);
  tcase_add_test(tc_core, test_cos_minus_inf);
  tcase_add_test(tc_core, test_cos_two_pi);
  tcase_add_loop_test(tc_core, test_cos_fractional_pi_values, -10, 10);
  tcase_add_loop_test(tc_core, test_cos_whole_pi_values, -10, 10);
